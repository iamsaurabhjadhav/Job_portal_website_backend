...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (!name || !email || !phone || !address || !password || !role) {
      return next(new ErrorHandler("All filelds are required.", 400));
    }
    ...
    if (req.files && req.files.resume) {
      ...
      if (resume) {
        try {
          const clouddinaryResponse = await clouddinary.uploader.upload(
            resume.tempFilePath,
            { folder: "Job_Seeker_Resume" }
          );
          if (!clouddinaryResponse || clouddinaryResponse.error) {
            return next(
          ...
          userData.resume = {
            ...
            url: clouddinaryResponse.secure_url,
          };
        } catch (error) {
...
export const updateUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.files) {
    ...
    if (resume) {
      ...
      if (!currentResumeId) {
        await clouddinary.uploader.destroy(currentResumeId);
      }
      const newResume = await clouddinary.uploader.upload(resume.tempFilePath, {
        folder: "Job_Seeker_Resume",
      });
      newUserData.resume = {
